...
bool qbXMLRPWrapper::OpenCompanyFile(string companyFile)
{
  ...
  if (! ((companyFile == m_CompanyFile) && (m_IsSessionOpen))) {
...
bool qbXMLRPWrapper::OpenQBConnection() {
  ...
  if (!m_IsConnectionOpen) {
    if (m_pReqProc == NULL) {
      ...
      HRESULT hr;
      hr = CoCreateInstance(CLSID_RequestProcessor,
                            NULL,
                            CLSCTX_INPROC_SERVER,
                            IID_IRequestProcessor,
                            reinterpret_cast<void**>(&m_pReqProc));
      if (FAILED(hr) || (m_pReqProc == NULL)) {
        TCHAR tmpMsg[1024];
        sprintf (tmpMsg, "Failed to instantiate the qbXML Request Processor.  Error: 0x%lx.  Perhaps qbxmlrp.dll is missing?", hr);
        m_ErrorMsg = tmpMsg;
        m_HasError = true;
        return false;
      }
