...
4) Point your IE browser to http://localhost/WCECommSample/client/Default.aspx 

5) Database ecommdb is pre-populated with some username/password (encryption not used for simplicity of the sample) used 
...
6) Database ecommdb is also pre-populated with some sample items (phone calling cards) that you could use for this sample. On the next update session with web connector any inventory items that you have on QB will be transfered to ecommdb. 

7) After you log on, do some purchasing (it's really a simulation and not tied to any payment system, so don't worry :-)) on client to create some txns (salesreceipts)
